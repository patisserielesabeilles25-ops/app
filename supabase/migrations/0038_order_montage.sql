-- 0038 — Montage (assembly/setup) price on orders
-- Nahla Cake Panel
--
-- Adds a per-order "montage" amount (prix de montage / التركيب) that counts
-- toward what the customer owes. The order grand total is now
--   total_amount + montage_amount
-- so remaining_amount and the advance/over-payment guards all include montage.
-- total_amount stays the cake/base price; montage is stored separately so the
-- printed invoice can show it on its own line.

-- ---------------------------------------------------------------------------
-- order_financials: add montage_amount, re-derive remaining, widen the guard.
...
alter table public.order_financials
  add column remaining_amount numeric(12,2)
    generated always as (total_amount + montage_amount - advance_payment) stored;

alter table public.order_financials
  add constraint advance_not_over_total
    check (advance_payment <= total_amount + montage_amount);

-- ---------------------------------------------------------------------------
-- create_order: accept the montage amount (grand total = total + montage).
-- ---------------------------------------------------------------------------
...
begin
  ...
  if p_total_amount < 0 or p_advance_payment < 0
     or coalesce(p_delivery_amount, 0) < 0 or v_montage < 0 then
    raise exception 'amounts cannot be negative';
  end if;
  if p_advance_payment > p_total_amount + v_montage then
  ...
  insert into public.order_financials (
    ...
    v_order_id, p_total_amount, p_advance_payment, coalesce(p_delivery_amount, 0), v_montage, v_uid, v_uid
  );
...
-- ---------------------------------------------------------------------------
-- record_order_payment: over-payment guard uses the grand total (incl. montage).
-- ---------------------------------------------------------------------------
...
begin
  ...
  select o.order_number, f.total_amount + f.montage_amount, f.advance_payment
